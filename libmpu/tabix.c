...
{
    _mpu_bgzf *fp = _mpu_bgzf_open(file,"r");
    if (_mpu_bgzf_read_block(fp) != 0 || !fp->block_length)
        return -1;
    ...
    _mpu_bgzf *_mpu_bgzf_out = _mpu_bgzf_fdopen(fileno(stdout), "w");
    ssize_t nread;
    while ( (nread=fread(buf,1,page_size-1,fh))>0 )
    {
        if ( nread<page_size-1 && buf[nread-1]!='\n' )
            buf[nread++] = '\n';
        if (_mpu_bgzf_write(_mpu_bgzf_out, buf, nread) < 0) error("Error: %s\n",_mpu_bgzf_out->error);
    }
    fclose(fh);

    if ( fp->block_length - skip_until > 0 )
    {
        if (_mpu_bgzf_write(_mpu_bgzf_out, buffer+skip_until, fp->block_length-skip_until) < 0)
            error("Error: %s\n",fp->error);
    }
    if (_mpu_bgzf_flush(_mpu_bgzf_out) < 0)
        error("Error: %s\n",_mpu_bgzf_out->error);
...
#ifdef _USE_KNETFILE
        int count = fwrite(buf, 1, nread, _mpu_bgzf_out->x.fpw);
#else
        int count = fwrite(buf, 1, nread, _mpu_bgzf_out->file);
#endif
        if (count != nread)
            error("Write failed, wrote %d instead of %d bytes.\n", count,(int)nread);
    }

    if (_mpu_bgzf_close(_mpu_bgzf_out) < 0)
        error("Error: %s\n",_mpu_bgzf_out->error);
...
int main(int argc, char *argv[])
{
	...
	if (optind + 1 == argc) {
        ...
        if ( _mpu_bgzf_check__mpu_bgzf(argv[optind])!=1 )
        {
