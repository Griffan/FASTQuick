...
/*
int ti_readline(_mpu_bgzf *fp, kstring_t *str)
{
	int c, l = 0;
	str->l = 0;
	while ((c = _mpu_bgzf_getc(fp)) >= 0 && c != '\n') {
		++l;
		if (c != '\r') kputc(c, str);
	}
...
int ti_readline(_mpu_bgzf *fp, kstring_t *str)
{
	...
	do {
		if (fp->block_offset >= fp->block_length) {
			if (_mpu_bgzf_read_block(fp) != 0) { state = -2; break; }
...
ti_index_t *ti_index_core(_mpu_bgzf *fp, const ti_conf_t *conf)
{
	int ret;
	ti_index_t *idx;
	...
	while ((ret = ti_readline(fp, str)) >= 0) {
		...
		if (_mpu_bgzf_tell(fp) <= last_off) {
			fprintf(stderr, "[ti_index_core] bug in _mpu_bgzf: %llx < %llx\n",
					(unsigned long long)_mpu_bgzf_tell(fp), (unsigned long long)last_off);
			exit(1);
		}
		last_off = _mpu_bgzf_tell(fp);
		last_coor = intv.beg;
	}
	if (save_tid >= 0) insert_offset(idx->index[save_tid], save_bin, save_off, _mpu_bgzf_tell(fp));
	merge_chunks(idx);
	fill_missing(idx);
...
void ti_index_save(const ti_index_t *idx, _mpu_bgzf *fp)
{
	int32_t i, size, ti_is_be;
	khint_t k;
	ti_is_be = bam_is_big_endian();
	_mpu_bgzf_write(fp, "TBI\1", 4);
	if (ti_is_be) {
		uint32_t x = idx->n;
		_mpu_bgzf_write(fp, bam_swap_endian_4p(&x), 4);
	} else _mpu_bgzf_write(fp, &idx->n, 4);
	assert(sizeof(ti_conf_t) == 24);
	if (ti_is_be) { // write ti_conf_t;
		uint32_t x[6];
		memcpy(x, &idx->conf, 24);
		for (i = 0; i < 6; ++i) _mpu_bgzf_write(fp, bam_swap_endian_4p(&x[i]), 4);
	} else _mpu_bgzf_write(fp, &idx->conf, sizeof(ti_conf_t));
	{ // write target names
		char **name;
		int32_t l = 0;
		...
		for (i = 0; i < kh_size(idx->tname); ++i)
			l += strlen(name[i]) + 1;
		if (ti_is_be) _mpu_bgzf_write(fp, bam_swap_endian_4p(&l), 4);
		else _mpu_bgzf_write(fp, &l, 4);
		for (i = 0; i < kh_size(idx->tname); ++i)
			_mpu_bgzf_write(fp, name[i], strlen(name[i]) + 1);
		free(name);
	}
	for (i = 0; i < idx->n; ++i) {
		khash_t(i) *index = idx->index[i];
		...
		if (ti_is_be) { // big endian
			uint32_t x = size;
			_mpu_bgzf_write(fp, bam_swap_endian_4p(&x), 4);
		} else _mpu_bgzf_write(fp, &size, 4);
		for (k = kh_begin(index); k != kh_end(index); ++k) {
			if (kh_exist(index, k)) {
				ti_binlist_t *p = &kh_value(index, k);
				if (ti_is_be) { // big endian
					uint32_t x;
					x = kh_key(index, k); _mpu_bgzf_write(fp, bam_swap_endian_4p(&x), 4);
					x = p->n; _mpu_bgzf_write(fp, bam_swap_endian_4p(&x), 4);
					for (x = 0; (int)x < p->n; ++x) {
						bam_swap_endian_8p(&p->list[x].u);
						bam_swap_endian_8p(&p->list[x].v);
					}
					_mpu_bgzf_write(fp, p->list, 16 * p->n);
					for (x = 0; (int)x < p->n; ++x) {
						bam_swap_endian_8p(&p->list[x].u);
						bam_swap_endian_8p(&p->list[x].v);
					}
				} else {
					_mpu_bgzf_write(fp, &kh_key(index, k), 4);
					_mpu_bgzf_write(fp, &p->n, 4);
					_mpu_bgzf_write(fp, p->list, 16 * p->n);
				}
			}
		}
		...
		if (ti_is_be) {
			int x = index2->n;
			_mpu_bgzf_write(fp, bam_swap_endian_4p(&x), 4);
		} else _mpu_bgzf_write(fp, &index2->n, 4);
		if (ti_is_be) { // big endian
			int x;
			for (x = 0; (int)x < index2->n; ++x)
				bam_swap_endian_8p(&index2->offset[x]);
			_mpu_bgzf_write(fp, index2->offset, 8 * index2->n);
			for (x = 0; (int)x < index2->n; ++x)
				bam_swap_endian_8p(&index2->offset[x]);
		} else _mpu_bgzf_write(fp, index2->offset, 8 * index2->n);
	}
}

static ti_index_t *ti_index_load_core(_mpu_bgzf *fp)
{
	int i, ti_is_be;
	char magic[4];
	ti_index_t *idx;
	...
	{ // read target names
		...
		int32_t l;
		uint8_t *buf;
		_mpu_bgzf_read(fp, &l, 4);
		if (ti_is_be) bam_swap_endian_4p(&l);
		buf = calloc(l, 1);
		_mpu_bgzf_read(fp, buf, l);
		str = calloc(1, sizeof(kstring_t));
		for (i = j = 0; i < l; ++i) {
			if (buf[i] == 0) {
	...
	for (i = 0; i < idx->n; ++i) {
		...
		// load linear index
		_mpu_bgzf_read(fp, &index2->n, 4);
		if (ti_is_be) bam_swap_endian_4p(&index2->n);
		index2->m = index2->n;
		index2->offset = (uint64_t*)calloc(index2->m, 8);
		_mpu_bgzf_read(fp, index2->offset, index2->n * 8);
		if (ti_is_be)
...
const char *ti_iter_read(_mpu_bgzf *fp, ti_iter_t iter, int *len)
{
	...
	while (1) {
		...
		if (iter->curr_off == 0 || iter->curr_off >= iter->off[iter->i].v) { // then jump to the next chunk
			...
			if (iter->i < 0 || iter->off[iter->i].v != iter->off[iter->i+1].u) { // not adjacent chunks; then seek
				_mpu_bgzf_seek(fp, iter->off[iter->i+1].u, SEEK_SET);
				iter->curr_off = _mpu_bgzf_tell(fp);
			}
			++iter->i;
		}
		if ((ret = ti_readline(fp, &iter->str)) >= 0) {
...
int ti_fetch(_mpu_bgzf *fp, const ti_index_t *idx, int tid, int beg, int end, void *data, ti_fetch_f func)
{
	ti_iter_t iter;
	const char *s;
	int len;
...
tabix_t *ti_open(const char *fn, const char *fnidx)
{
	tabix_t *t;
	_mpu_bgzf *fp;
	if ((fp = _mpu_bgzf_open(fn, "r")) == 0) return 0;
