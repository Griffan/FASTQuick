...
class wFile {
 ...
 wFile() : fp(NULL), bgf(NULL), buf(NULL) {}

  void open(const char* filename, int mode = -1)  {
   if ( mode == -1 ) { // use _mpu_bgzf if ends with .gz, otherwise, plain mode
   ...
   case 2:
     fp = NULL;
     gf = NULL;
     bgf = _mpu_bgzf_open(filename,"wb");
     if ( bgf == NULL ) {
  ...
  int write(void* ptr, size_t count) {
    ...
    case 2: // bgzipped files
      return _mpu_bgzf_write(bgf, ptr, count);
    default:
  ...
  int putc(char c) {
    ...
    case 1:
      return gzputc(gf,c);
    case 2:
      return _mpu_bgzf_write(bgf, (void*)&c, 1);
    default:
  ...
  int puts(const char* s) {
    ...
    case 2:
      n = strlen(s);
      return _mpu_bgzf_write(bgf, s, n);
    default:
  ...
  void close() {
    ...
    case 1:
      ...
      break;
    case 2:
      if ( bgf != NULL ) { _mpu_bgzf_close(bgf); bgf = NULL; }
      break;
    }
  }
};
