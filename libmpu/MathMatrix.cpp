...
void Matrix::Product(const Matrix & l, const Matrix & r)
{
    if (l.cols != r.rows)
        error("Matrix.Multiply - Attempted to multiply incompatible matrices\n"
              "Matrices        - %s [%d, %d] + %s [%d, %d]\n",
              l.label.c_str(), l.rows, l.cols,
              r.label.c_str(), r.rows, r.cols);
...
void Matrix::Print(FILE * f, int r, int c)
{
    ...
    for (int j = 0; j < c; j++)
    {
        ...
        fprintf(f, "%*s ", width[j], extras[j].label.c_str());
    }

    for (int i = 0; i < r; i++)
    {
      fprintf(f, "\n%*s ", columnZero, data[i]->label.c_str());
        for (int j = 0; j < c; j++)
...
void Matrix::CopyLabels(Matrix & M)
{
    for (int i = 0; i < rows; i++)
        if (i < M.rows)
	  data[i]->SetLabel(M[i].label.c_str());
...
void Matrix::PrintLower(FILE * f, int r, int c, bool print_diag)
{
    ...
    char buf[255];
    sprintf(buf, "[%d x %d]", r, c);
    dimensions = buf;

    int columnZero = label.size() > 15 ? label.size() : 15;

    fprintf(f, "\n%*s =\n%*s ", columnZero, label.c_str(),
            columnZero, dimensions.c_str());
...
void Matrix::SetupPrint(FILE *f, int r, int c, int & column_zero, int * precision, int * width)
{
    ...
    column_zero = label.size() > 15 ? label.size() : 15;

    fprintf(f, "\n%*s =\n%*s ", column_zero, label.c_str(),
            column_zero, dimensions.c_str());
