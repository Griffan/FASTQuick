...
class pFile {
 protected:
  ...
  //static const int MAX_LINE_SIZE = 65536L; //10000000L;
 ...
 public:
  ...
  // return 0 if not gzip, 1 if gz but not _mpu_bgzf, 2 if _mpu_bgzf, -1 if 10 bytes cannot be read
  ...
  void load(const char* filename, const char* region = NULL, bool printHeader = false, bool no_mpu_bgzf = false) {
    head = printHeader;
    if ( region != NULL ) reg = region;
    open(filename, no_mpu_bgzf);
    if ( reg.empty() ) {
 ...
 pFile(const char* filename, const char* region = NULL, bool printHeader = false) : head(printHeader), iter(NULL), type(-1), line(NULL), idxconf(NULL) {
  ...
  void updateRegion(const char* region, bool sepchr = false) {
    //notice("foo updateRegion(%s)",region);
    reg = region;
    if ( reg.empty() ) {
      loadAll();
    }
    ...
    //notice("bar updateRegion(%s)",region);
  }
  ...
  int read(void* ptr, size_t count) {
    ...
    case 2: // bgzipped files
      return _mpu_bgzf_read(t->fp, ptr, count);
    default:
  ...
  const char* getLine() {
    ...
    case 1:
      if ( line == NULL ) line = new char[MAX_LINE_SIZE];
            if (gzgets(gf, line, MAX_LINE_SIZE) != nullptr) {
	len = strlen(line); // TODO : convert to lazy evaluation
	if ( line[len-1] == '\n' ) {
	  line[len-1] = '\0';
	  --len;
  ...
  void loadIndex() {
    //notice("foo loadIndex() %x",t);
    if (ti_lazy_index_load(t) < 0 ) {
      error("Failed to load the index file of %s",fname.c_str());
    }
    //notice("bar loadIndex() %x",t);
