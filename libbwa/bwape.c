...
int pairing(bwa_seq_t *p[2], pe_data_t *d, const pe_opt_t *opt, int s_mm, const isize_info_t *ii)
{
	...
	if (max_len < p[1]->full_len) max_len = p[1]->full_len;// setting max_len to be the longer end's length
	if (low_bound < max_len) low_bound = max_len;// setting insert size lower bound to longer end's length
	...
	for (j = 0; j < 2; ++j) last_pos[j][0] = last_pos[j][1] = (uint64_t)-1;// for each pair end
	if (opt->type == BWA_PET_STD) {
		for (i = 0; i < d->arr.n; ++i) {//foreach hit
			uint64_t x = d->arr.a[i];//primary index, lower bound, which end packed struct
			...
			if (strand == 1) { // reverse strand, then check
				...
				last_pos[x & 1][0] = last_pos[x & 1][1];//keep records of last two position
				last_pos[x & 1][1] = x;
			}
		}
	}
...
fread(&n_aln, 4, 1, fp_sa[j]);// read in total number of aln
