...
int runBGZIndex(int argc, char** argv) {
  ...
  if ( indexFlag ) {
    ...
    /* BGZF* */_mpu_bgzf* bp = _mpu_bgzf_open(gzf.c_str(),"r");

    wFile wf(outf.c_str());
    int c;
    
    if (_mpu_bgzf_seek(bp, start, SEEK_SET) < 0) error("Cannot perform bgzf_seek in %s to %ld",gzf.c_str(),start);
    while (1) {
      if (end < 0) c = _mpu_bgzf_read(bp, buffer, WINDOW_SIZE);
      else c = _mpu_bgzf_read(bp, buffer, (end - start > WINDOW_SIZE)? WINDOW_SIZE:(end - start));
      if (c == 0) break;
