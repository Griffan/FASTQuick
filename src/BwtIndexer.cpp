...
#ifdef KMER_8_BIT
unsigned int mask[6] = {240, // 11110000
                        15,  // 00001111
                        195, // 11000011
                        60,  // 00111100
                        204, // 11001100
...
#endif
#ifdef KMER_16_BIT
unsigned int mask[6] = {65280, // 1111111100000000
...
#endif
#ifdef KMER_32_BIT
unsigned int mask[6] = {0xffff0000, 0xffff,     0xff0000ff,
                        0xffff00,   0xff00ff00, 0xff00ff};
#endif
#ifdef KMER_64_BIT
// const uint64_t mask[6] =
//        {0xffffffff00000000, 0xffffffff, 0xffff00000000ffff, 0xffffffff0000,
//        0xffff0000ffff0000, 0xffff0000ffff};
#endif

#define NST_NT4_TABLE(X)                                                       \
  (nst_nt4_table[(unsigned char)X] < 4 ? nst_nt4_table[(unsigned char)X]       \
                                       : rand() % 4)
...
BwtIndexer::BwtIndexer()
  ...
  parameters.projected_element_count =
      VCF_SITES * (WINDOW_SIZE - KMER_SIZE + 1 + KMER_SIZE) * 2;
...
inline bool BwtIndexer::IsKmerInHash(uint64_t kmer) const {
  ...
  //{
  //	//printf("the DATUM is : %016llx    masked DATUM:%x    the hash value is
  //:%d as well as mask:%x\n",kmer,KmerShrinkage(kmer,
  // iter),roll_hash_table[iter][KmerShrinkage(kmer, iter)], iter); 	uint32_t
  // shrinked = KmerShrinkage(kmer, iter); 	if
  ...
  // shrinked);
  if (roll_hash_table[0][shrinked / 8] & (1 << (shrinked % 8))) {
    counter++;
  }
  /*0xffffffff*/
  shrinked = 0;
  ...
  if (roll_hash_table[1][shrinked / 8] & (1 << (shrinked % 8))) {
    counter++;
  }
  /*0xffff00000000ffff*/
  shrinked = 0;
  ...
  // shrinked);
  if (roll_hash_table[2][shrinked / 8] & (1 << (shrinked % 8))) {
    counter++;
  }
  /*0xffffffff0000*/
  shrinked = 0;
  ...
  if (roll_hash_table[3][shrinked / 8] & (1 << (shrinked % 8))) {
    counter++;
  }
  /*0xffff0000ffff0000*/
  shrinked = 0;
  ...
  // shrinked);
  if (roll_hash_table[4][shrinked / 8] & (1 << (shrinked % 8))) {
    counter++;
  }
  /*0xffff0000ffff*/
  shrinked = 0;
  ...
  if (roll_hash_table[5][shrinked / 8] & (1 << (shrinked % 8))) {
    counter++;
  }
...
bool BwtIndexer::IsReadInHash(const ubyte_t *S, int len,
                              bool more_chunck) const {
  //	for (int i = 0; i != len;++i)
...
bool BwtIndexer::IsReadInHash(ubyte_t *S, int len) const {
  uint64_t kmer3[2];
  v16qi mmx1 = {S[0],  S[4],  S[8],  S[12], S[16], S[20], S[24], S[28],
                S[32], S[36], S[40], S[44], S[48], S[52], S[56], S[60]};
  v16qi mmx2 = {S[1],  S[5],  S[9],  S[13], S[17], S[21], S[25], S[29],
                S[33], S[37], S[41], S[45], S[49], S[53], S[57], S[61]};
  v16qi mmx3 = {S[2],  S[6],  S[10], S[14], S[18], S[22], S[26], S[30],
                S[34], S[38], S[42], S[46], S[50], S[54], S[58], S[62]};
  v16qi mmx4 = {S[3],  S[7],  S[11], S[15], S[19], S[23], S[27], S[31],
                S[35], S[39], S[43], S[47], S[51], S[55], S[59], S[63]};
  v16qi mmx5 = {S[64], S[68], S[72], S[76], S[80], S[84], S[88], S[92],
                0,     0,     0,     0,     0,     0,     0,     0};
  v16qi mmx6 = {S[65], S[69], S[73], S[77], S[81], S[85], S[89], S[93],
                0,     0,     0,     0,     0,     0,     0,     0};
  v16qi mmx7 = {S[66], S[70], S[74], S[78], S[82], S[86], S[90], S[94],
                0,     0,     0,     0,     0,     0,     0,     0};
  v16qi mmx8 = {S[67], S[71], S[75], S[79], S[83], S[87], S[91], S[95],
                0,     0,     0,     0,     0,     0,     0,     0};
...
#define GCC_VERSION                                                            \
  ...
  mmx1 = mmx1 | (mmx3 << 2);
#else
  v16qi mul64 = {64, 64, 64, 64, 64, 64, 64, 64,
                 64, 64, 64, 64, 64, 64, 64, 64};
  v16qi mul16 = {16, 16, 16, 16, 16, 16, 16, 16,
  ...
  // for (int i = 0; i != 32; ++i)
  //	fprintf(stderr, "%c\t", "ACGT"[(swap_uint64(kmer3[0])>> ((31 - i) * 2))
  //& 3]);
  ////fprintf(stderr, "\n");
  // for (int i = 0; i != 32; ++i)
  //	fprintf(stderr, "%c\t", "ACGT"[(swap_uint64(kmer3[1]) >> ((31 - i) * 2))
  //& 3]); memcpy(&kmer3, &mmx5, 16); for (int i = 0; i != 32; ++i)
  //	fprintf(stderr, "%c\t", "ACGT"[(swap_uint64(kmer3[0]) >> ((31 - i) * 2))
  //& 3]); fprintf(stderr, "\n"); memcpy(&kmer3, &mmx1, 16); fprintf(stderr,
  // "0-31: %016llx\t", swap_uint64(kmer3[0])); fprintf(stderr, "32-63:
  // %016llx\n", swap_uint64(kmer3[1])); fprintf(stderr, "%d\t", (mmx1[0] >> 2)
  // & 3);
  ...
  if (IsKmerInHash(swap_uint64(kmer3[0])) ||
      IsKmerInHash(swap_uint64(kmer3[1]))) {
...
bool BwtIndexer::IsReadInHashByCountMoreChunck(const ubyte_t *S,
                                               int len) const {
...
bool BwtIndexer::IsReadFiltered(ubyte_t *S, const ubyte_t *Q, int len) const {
  ...
  {
    if ((len > 0 ? IsReadInHashByCountMoreChunck(S, len)
                 : IsReadInHashByCount(S, len))) {
      return false;
    } else {
      return true;
    }
...
void BwtIndexer::InitializeRollHashTable(int thresh = 3) {
  ...
  for (int i = 0; i != 6; ++i)
    roll_hash_table[i] = (unsigned char *)calloc(hash_table_size, sizeof(char));
...
void BwtIndexer::DumpRollHashTable(const std::string &prefix) {
  ...
  for (int i = 0; i != 6; ++i)
    fout.write(reinterpret_cast<char *>(roll_hash_table[i]), hash_table_size);
  fout.close();
...
void BwtIndexer::AddSeq2HashCore(const std::string &Seq, int iter,
  ...
  for (; i != Seq.length() / 2 /*last one considered*/; ++i) {
    // std::cerr<<"number:"<<i<<"COmparing
    // length:"<<KMER_SIZE<<"and"<<Seq.length()<<"while
    // max:"<<Seq.max_size()<<std::endl;
    datum = ((datum << 2) | NST_NT4_TABLE((unsigned char) Seq[i]));/*
		 & LOMEGA(
		 min_only(RollParam.kmer_size,OVERFLOWED_KMER_SIZE));*/ // N not considered
  ...
  // printf("the DATUM is : %016llx    masked DATUM:%x    the hash value is :%d
  // as well as mask:%x\n",datum,KmerShrinkage(datum,
  // iter),roll_hash_table[iter][KmerShrinkage(datum,
  // iter)/8]&(1<<(shrinked%8)), iter);
  ...
  for (unsigned int j = i, let = 0; let != alleles.size(); ++let, j = i) {
    ...
    for (; j != Seq.length() / 2 + 32; ++j) {
      ...
      // if (debug_flag)
      //{
      //	fprintf(stderr, "I found it!!!!!\t%c\t",
      // nst_nt4_table["ACGT"[(char)let]]); 	for (int k = 0; k != 32; ++k)
      //		fprintf(stderr, "%c", "ACGT"[(datum >> ((31 - k) * 2)) &
      // 3]); 	fprintf(stderr, "\t"); 	for (int k = 0; k != 16; ++k)
      //		fprintf(stderr, "%c", "ACGT"[(shrinked >> ((15 - k) *
      // 2)) & 3]); 	fprintf(stderr, "\t%llx\n", mask[iter]);
      //}
      roll_hash_table[iter][shrinked / 8] |= (1 << (shrinked % 8));
      // AddSeq2DebugHash(shrinked);
    }
  ...
  // printf("the DATUM is : %x    the hash value is :%d as well
  // as:%x\n",datum,roll_hash_table[datum],
  // LOMEGA(min_only(RollParam.kmer_size,OVERFLOWED_KMER_SIZE)));
}

/******************************/
bool BwtIndexer::BuildIndex(RefBuilder &ArtiRef, std::string &OldRef,
                            std::string &NewRef, const gap_opt_t *opt) {
  // NewRef += ".FASTQuick.fa";
  RefPath = OldRef;
  string str;
...
bool BwtIndexer::Fa2Pac(const char *prefix) {
  // initialization
  // seq = kseq_init(fp_fa);
  char name[1024];
  ...
  while (std::getline(FIN, CurrentSeqName)) {
    CurrentSeqName.erase(CurrentSeqName.begin());
    ...
    // if (debug_flag) std::cerr << string(CurrentSeq.rbegin(),
    // CurrentSeq.rend()) << endl; AddSeq2Hash(string(CurrentSeq.rbegin(),
    // CurrentSeq.rend()), alleles);
    string tmp_rev_cmp = ReverseComplement(CurrentSeq);
    // if (debug_flag) std::cerr << tmp_rev_cmp << endl;
    AddSeq2Hash(tmp_rev_cmp, alleles);
    // if (debug_flag) std::cerr << string(tmp_rev_cmp.rbegin(),
    // tmp_rev_cmp.rend()) << endl; AddSeq2Hash(string(tmp_rev_cmp.rbegin(),
    // tmp_rev_cmp.rend()), alleles); if (debug_flag) exit(EXIT_FAILURE);
    // DBG(fprintf(stderr,"Come into outer
    ...
    // p->anno = seq->comment.s? strdup((char*)seq->comment.s) :
    // strdup("(null)");
    p->anno = strdup("(null)");
    p->gi = 0;
    ...
    p->offset =
        (bns->n_seqs == 0)
            ? 0
            : (p - 1)->offset + (p - 1)->len; // start position of new sequence
    p->n_ambs = 0;
    for (i = 0, lasts = 0; i < CurrentSeq.length(); ++i) {
      int c = nst_nt4_table[(int)CurrentSeq[i]];
      if (c >= 4) {                   // N
        if (lasts == CurrentSeq[i]) { // contiguous N
          ++q->len;
        } else {
          if (bns->n_holes == m_holes) {
            m_holes <<= 1;
            bns->ambs =
                (bntamb1_t *)realloc(bns->ambs, m_holes * sizeof(bntamb1_t));
          }
      ...
      { // fill buffer
        ...
        if (l_buf == m_pac_buf) {
          ...
          ubyte_t *tmp_pac_buf =
              (unsigned char *)calloc(tmp_size_pac_buf, sizeof(unsigned char));
          memcpy(tmp_pac_buf, pac_buf, size_pac_buf);
          free(pac_buf);
          pac_buf = tmp_pac_buf;
          size_pac_buf = tmp_size_pac_buf;

          // pac_buf = (unsigned char*)realloc(pac_buf, size_pac_buf *
          // sizeof(char));
        }
...
bool BwtIndexer::BuildIndexFromWholeGenome(RefBuilder &ArtiRef,
                                           std::string &OldRef,
                                           std::string &NewRef,
                                           const gap_opt_t *opt) {
  // NewRef += ".FASTQuick.fa";
  RefPath = OldRef;
  std::string str, str2;
  FillHashTable(NewRef.c_str());
  DumpRollHashTable(NewRef);

  //    str=RefPath+".pac";
  //    if (stat(str.c_str(), &sb) == 0) {//existed
  //        notice("%s has already existsed, skip indexing...",str.c_str());
  //        return true;
  //    }

  gzFile fp = xzopen(RefPath.c_str(), "r");
  fprintf(stderr, "[bwa_index] Pack FASTA...\n");
  bns_fasta2bntseq(fp, RefPath.c_str());
  gzclose(fp);
  {
    str = RefPath + ".pac";
    str2 = RefPath + ".rpac";
  ...
  }
  {
    ...
    bwt_bwtgen(str.c_str(), str2.c_str());
  }
  {
  ...
  }
  {
    ...
    bwt = bwt_restore_bwt(str.c_str());
    bwt_bwtupdate_core(bwt);
    bwt_dump_bwt(str.c_str(), bwt);
    bwt_destroy(bwt);
  }
  {
    bwt_t *bwt;
...
bool BwtIndexer::LoadIndexFromWholeGenome(string &NewRef) {
  std::string str;
...
bool BwtIndexer::Fa2PacFromWholeGenome() {
  ...
  q = bns->ambs;
  l_buf = 0;            // base counter
  m_pac_buf = 0x800000; // limit of number of bases
  uint64_t size_pac_buf = 0x200000;
  pac_buf = (unsigned char *)calloc(size_pac_buf, sizeof(unsigned char));
  strcpy(name, RefPath.c_str());
  strcat(name, ".pac");
  fp = xopen(name, "wb");
  // read sequences
  // while ((l = kseq_read(seq)) >= 0) {
  DBG(fprintf(stderr, "NOTE:Come into Fa2Pac...\n");)
  // RefInput+=".FASTQuick.fa";
  ifstream FIN(RefPath);
  string CurrentSeqName, CurrentSeqNameAnno;
  string CurrentSeq;
  string CurrentLine;
  unsigned long pos;
  // initialize first line
  std::getline(FIN, CurrentSeqName);
  CurrentSeqName.erase(CurrentSeqName.begin()); // remove '>'
  pos = CurrentSeqName.find_first_of("\t\n ");
  CurrentSeqNameAnno = CurrentSeqName.substr(pos + 1);
  CurrentSeqName = CurrentSeqName.substr(0, pos);
  CurrentSeq = "";
  while (std::getline(FIN, CurrentLine)) {
    if (CurrentLine[0] != '>') {
      CurrentSeq += CurrentLine;
    } else {
      ConvertSeq2Pac(m_seqs, m_holes, q, size_pac_buf, CurrentSeqName,
                     CurrentSeqNameAnno, CurrentSeq);
      // new chromosome
      CurrentSeqName = CurrentLine;
      CurrentSeqName.erase(CurrentSeqName.begin()); // remove '>'
      pos = CurrentSeqName.find_first_of("\t\n ");
      CurrentSeqNameAnno = CurrentSeqName.substr(pos + 1);
      CurrentSeqName = CurrentSeqName.substr(0, pos);
  ...
  free(pac_buf); // not free in non-human version function
  pac_buf = NULL;
  //    bns->to_pac_buf = pac_buf;
  // DBG(fprintf(stderr,"l_pac: %d\nbns->pac_buf:
  // %x\n",bns->l_pac,bns->to_pac_buf);)
...
void BwtIndexer::ConvertSeq2Pac(uint32_t &m_seqs, uint32_t &m_holes,
  ...
  p->anno = strdup(CurrentSeqNameAnno.c_str());
  p->gi = 0;
  p->len = CurrentSeq.length(); // length of the sequence
  p->offset =
      (bns->n_seqs == 0)
  ...
  for (i = 0, lasts = 0; i < CurrentSeq.length(); ++i) {
    ...
    { // fill buffer
      ...
      if (l_buf == m_pac_buf) {
        //                        m_pac_buf += 0x40000;
        //                        int32_t tmp_size_pac_buf = size_pac_buf +
        //                        0x10000;

        //                        m_pac_buf += 0x800000;
        //                        uint64_t tmp_size_pac_buf = size_pac_buf +
        //                        0x200000; ubyte_t *tmp_pac_buf = (ubyte_t *)
        //                        calloc(
        //                                tmp_size_pac_buf, sizeof(unsigned
        //                                char));
        //                        memcpy(tmp_pac_buf, pac_buf, size_pac_buf);
        //                        free(pac_buf);
        //                        pac_buf = tmp_pac_buf;
        ...
        m_pac_buf += 0x800000;
        size_pac_buf += 0x200000;
        pac_buf =
            (unsigned char *)realloc(pac_buf, size_pac_buf * sizeof(char));
        memset(pac_buf + bns->l_pac / 4, 0, (m_pac_buf - bns->l_pac) / 4);
      }
      pac_buf[l_buf >> 2] |= c << ((3 - (l_buf & 3)) << 1);
      ++l_buf;
    }
...
bool BwtIndexer::FillHashTable(const char *prefix) {
  DBG(fprintf(stderr, "NOTE:Come into Fa2Pac...\n");)
  string RefInput(prefix);
  ifstream FIN(RefInput);
  string CurrentSeqName;
...
bool BwtIndexer::Fa2RevPac(const char *prefix) { // TODO: check usage of j
  int64_t seq_len, i;
  bwtint_t pac_len, j;
