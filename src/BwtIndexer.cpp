...
bool BwtIndexer::IsReadInHash(ubyte_t *S, int len) const {
    uint64_t kmer3[2];
    v16qi mmx1 = {S[0], S[4], S[8], S[12], S[16], S[20], S[24], S[28], S[32], S[36], S[40], S[44], S[48], S[52], S[56],
                  S[60]};
    v16qi mmx2 = {S[1], S[5], S[9], S[13], S[17], S[21], S[25], S[29], S[33], S[37], S[41], S[45], S[49], S[53], S[57],
                  S[61]};
    v16qi mmx3 = {S[2], S[6], S[10], S[14], S[18], S[22], S[26], S[30], S[34], S[38], S[42], S[46], S[50], S[54], S[58],
                  S[62]};
    v16qi mmx4 = {S[3], S[7], S[11], S[15], S[19], S[23], S[27], S[31], S[35], S[39], S[43], S[47], S[51], S[55], S[59],
                  S[63]};
...
bool BwtIndexer::BuildIndexFromWholeGenome(RefBuilder &ArtiRef, std::string &OldRef, std::string &NewRef,
                                           const gap_opt_t *opt) {
    //NewRef += ".FASTQuick.fa";
    RefPath = OldRef;
    std::string str, str2;
...
bool BwtIndexer::Fa2PacFromWholeGenome() {
    ...
    while (std::getline(FIN, CurrentLine)) {
        if (CurrentLine[0] != '>') {
            ...
            ConvertSeq2Pac(m_seqs, m_holes, q, size_pac_buf, CurrentSeqName, CurrentSeqNameAnno, CurrentSeq);
            //new chromosome
...
void BwtIndexer::ConvertSeq2Pac(uint32_t &m_seqs, uint32_t &m_holes, bntamb1_t *q, uint64_t &size_pac_buf,
                                const string &CurrentSeqName, const string &CurrentSeqNameAnno,
...
//                        m_pac_buf += 0x800000;
//                        uint64_t tmp_size_pac_buf = size_pac_buf + 0x200000;
//                        ubyte_t *tmp_pac_buf = (ubyte_t *) calloc(
//                                tmp_size_pac_buf, sizeof(unsigned char));
//                        memcpy(tmp_pac_buf, pac_buf, size_pac_buf);
//                        free(pac_buf);
//                        pac_buf = tmp_pac_buf;
