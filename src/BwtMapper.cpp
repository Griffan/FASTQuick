...
BwtMapper::BwtMapper(BwtIndexer &BwtIndex, const string &Fastq_1,
                     ...
                     gap_opt_t *opt) : bwa_rg_line(nullptr), bwa_rg_id(nullptr) {
    ...
    if (opt->in_bam != 0) {
        ...
        while (getline(fin, line)) {
            ...
            if (Fastq_2 != "") {
                ...
                t_tmp = realtime();
                /*PairEndMapper*//*PairEndMapper_dev*/ PairEndMapper_without_asyncIO(BwtIndex, Fastq_1.c_str(),
                                                                                     Fastq_2.c_str(), popt, opt, SFH,
                                                                                     BamIO, BamFile, StatusTracker,
                                                                                     fout, total_add);
...
static bwa_seq_t *
bwa_read_seq_with_hash(BwtIndexer *BwtIndex, bwa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual,
                       double frac, uint32_t seed) {
    ...
    while (1) {
        ...
        if (l_bc) { // then trim barcode
            for (i = 0; i < l_bc; ++i)
                p->bc[i] = (seq->qual.l && seq->qual.s[i] - 33 < BARCODE_LOW_QUAL) ? tolower(seq->seq.s[i]) : toupper(
                        seq->seq.s[i]);
            p->bc[i] = 0;
        ...
        seq_reverse(p->len, p->seq,
                    0); // *IMPORTANT*: will be reversed back in bwa_refine_gapped()//reversing here might affect hash filtering result comparing to old version that put hash after this
...
static int
bwa_read_seq_with_hash_dev(BwtIndexer *BwtIndex, bwa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual,
                           double frac, uint32_t seed, bwa_seq_t *seqs, int &read_len) {
...
static void *IOworker(void *data) {
    ...
    *(d->ret) = bwa_read_seq_with_hash_dev(d->BwtIndex, d->ksAddress, READ_BUFFER_SIZE, d->n_seqs, d->mode,
                ...
                for (k = 0; k < d->aln[j].n; ++k) { // for each alignment
                    ...
                    if (r->l - r->k + 1 >= MIN_HASH_WIDTH) { // then check hash table
                        ...
                        if (ret) { // if this key is not in the hash table; ret must equal 1 as we never remove elements
                            ...
                            for (l = r->k; l <= r->l; ++l)
                                z->a[l - r->k] = r->a ? bwt_sa(bwt[0], l) : bwt[1]->seq_len - (bwt_sa(bwt[1], l) +
                                                                                               p[j]->len); //call forward / reverse bwt respectively
                        }
                        for (l = 0; (int) l < kh_val(g_hash,
                                                     iter).n; ++l) { // ret will surelly show this key in hash, just get its value
...
bool BwtMapper::SingleEndMapper(BwtIndexer &BwtIndex, const char *fn_fa,
                                const gap_opt_t *opt, SamFileHeader &SFH, BamInterface &BamIO, IFILE BamFile,
...
//			}
        ...
        if (!opt->out_bam) {
            for (i = 0; i < n_seqs; ++i) {
                ...
                //if (
                total_add += collector.AddAlignment(BwtIndex.bns, seqs + i, 0, opt, fout, total_dup);
                //==0)
                //continue; //failed
...
bool BwtMapper::PairEndMapper_without_asyncIO(BwtIndexer &BwtIndex, const char *fn_fa1, const char *fn_fa2,
...
//	opt->read_len = kseq_get_seq_len_fpc(ks[0]->ks);
    ...
    while (ReadIsGood) { // opt should be different for two fa files theoretically
        ...
        if (opt->n_threads <= 1) { // no multi-threading at all
            ...
            round++;
            if ((ret = bwa_read_seq_with_hash_dev(&BwtIndex, ks[0], READ_BUFFER_SIZE, &n_seqs_buff[0], opt->mode,
        ...
        if (!opt->out_bam) {
            for (i = 0; i < n_seqs[0]; ++i) {
                ...
                // if (
                total_add += collector.AddAlignment(BwtIndex.bns, seqs[0] + i, seqs[1] + i, opt,
                                                    fout, total_dup);
                //							  ==0)
                // continue;
                bwa_print_sam1(BwtIndex.bns, p[0], p[1], opt->mode, opt->max_top2);
