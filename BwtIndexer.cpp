...
bool BwtIndexer::IsKmerInHash(uint64_t kmer)
{
	int counter(0);
	uint32_t shrinked(0);
...
bool BwtIndexer::IsReadFiltered(const ubyte_t * S, const ubyte_t * Q, int len)
{
...
uint32_t BwtIndexer::KmerShrinkage(uint64_t a, unsigned int mask)
{
	uint32_t tmp(0), i(1);
