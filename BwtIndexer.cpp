...
BwtIndexer::BwtIndexer():pac_buf(0),rpac_buf(0),bwt_buf(0),rbwt_buf(0),bns(0), l_buf(0),m_pac_buf(0),m_rpac_buf(0),m_bwt_buf(0),m_rbwt_buf(0),bwt_d(0),rbwt_d(0)
{
...
BwtIndexer::BwtIndexer(RefBuilder & ArtiRef,string & prefix):pac_buf(0),rpac_buf(0),bwt_buf(0),rbwt_buf(0),bns(0), l_buf(0),m_pac_buf(0),m_rpac_buf(0),m_bwt_buf(0),m_rbwt_buf(0),bwt_d(0),rbwt_d(0)
{
#ifdef BLOOM_FPP
	//***bloom filter initialization
...
BwtIndexer::BwtIndexer(string & prefix):pac_buf(0),rpac_buf(0),bwt_buf(0),rbwt_buf(0),bns(0), l_buf(0),m_pac_buf(0),m_rpac_buf(0),m_bwt_buf(0),m_rbwt_buf(0),bwt_d(0),rbwt_d(0)
{
#ifdef BLOOM_FPP
	//***bloom filter initialization
...
bool BwtIndexer::BuildIndex(RefBuilder & ArtiRef,string & prefix, const gap_opt_t * opt)
{
...
bool BwtIndexer::Fa2Pac(RefBuilder & ArtiRef, const char *prefix, const gap_opt_t* opt)
{
	// initialization
	//seq = kseq_init(fp_fa);
	char name[1024];
	...
	for(unordered_map<string,uint32_t >::iterator iter=ArtiRef.RefTableIndex.begin(); iter!=ArtiRef.RefTableIndex.end(); ++iter)
	{
		...
		p->anno =  strdup("(null)");
		p->gi = 0; p->len = CurrentSeq.length();//length of the sequence
		...
		for (i = 0, lasts = 0; i < CurrentSeq.length(); ++i) {
			...
			{ // fill buffer
				...
				if (l_buf ==m_pac_buf)
				{
					...
					ubyte_t * tmp_pac_buf=(unsigned char *)calloc(tmp_size_pac_buf,sizeof(unsigned char));
...
bool BwtIndexer::Fa2RevPac(const char * prefix)
{
	...
	FILE *fp;
	char name[1024];
	strcpy(name, prefix); strcat(name, ".rpac");
	seq_len = bns->l_pac;
	pac_len = (seq_len >> 2) + 1;
	bufin = pac_buf;
...
bwt_t*  BwtIndexer::Pac2Bwt(unsigned char * pac)
{
		...
		for (i = 2; i <= 4; ++i) bwt->L2[i] += bwt->L2[i-1];//cumulative
		//free(buf2);
...
BwtIndexer::~BwtIndexer()
{
	...
	free(bwt_d->sa); free(bwt_d->bwt);
	free(bwt_d);
	DBG(fprintf(stderr,"bwt_d delete successfully...\n");)
	free(rbwt_d->sa); free(rbwt_d->bwt);
	free(rbwt_d);
	DBG(fprintf(stderr,"rbwt_d delete successfully...\n");)
	if(pac_buf) free( pac_buf);
	if(rpac_buf) free( rpac_buf);
	if(bwt_buf) free(bwt_buf);
	if(rbwt_buf) free(rbwt_buf);
